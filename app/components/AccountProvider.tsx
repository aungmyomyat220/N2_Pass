...
type User = { id: string; name: string; email?: string | null; image?: string | null };
type Account = { user: User | null; configured: boolean; state: StudyState; revision: number };
type AccountUI = { sidebar: ReactNode; profile: ReactNode };
const Context = createContext<AccountUI>({ sidebar: null, profile: null });
export function AccountControls() { return useContext(Context).sidebar; }
export function AccountProfile() { return useContext(Context).profile; }

function GoogleIcon() {
  return <svg className="google-sign-in-icon" viewBox="0 0 24 24" aria-hidden="true">
...
export default function AccountProvider({ children }: { children: ReactNode }) {
  ...
  const controls = <section className="account-controls" aria-label="Account">
    ...
    {account?.user?.email && <span className="account-email">{account.user.email}</span>}
    <small role="status">{status}</small>
    {account?.user ? <>
      <button disabled={busy} onClick={() => void logout()}><LogOut aria-hidden="true" /> Sign out</button>
  ...
  const profile = <HoverCard>
    ...
    <HoverCardContent align="end" side="bottom" sideOffset={8} className="account-profile-card">
      {controls}
    </HoverCardContent>
  </HoverCard>;
